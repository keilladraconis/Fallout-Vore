...
Event Actor.OnPlayerLoadGame(actor aSender)
	IsDefWSur1Loaded()	;Check if Def Survival UI is installed
...
SendAdrenalineUpdateEvent() ;/ "Survival stats widget" /;
EndFunction
...
Function ApplyEffect(GlobalVariable RuleGlobal, Effect[] EffectsArray, ActorValue EffectActorValue, bool DispellRestedSpells = false, int IncrementEffectBy = 0, bool DisplayAfterAwakingMessage = false, bool bDisplayMessage = true, bool bDamageFatigue = true, bool bAnnounceFatigue = false, bool bBypassGlobalCheck = false)
	...
	If (Pdef_W_SUR1_enabled)
		if (Pdef_W_SUR1_statusmessages.GetValue() as bool && bDisplayMessage && EffectMessageToShow != PreviousEffectMessageToShow) ;/ "Survival stats widget" /;
...
Function ProcessSingleFoodItem(Form akBaseObject)
	...
	if akBaseObject.HasKeyword(ObjectTypeCaffeinated)
		...
		SendCafUpdateEvent(timeToCaffeinate,CaffeinatedCount,ExtraCaffeinatedCount) ;/ "Survival stats widget" /;
	...
	if akBaseObject.HasKeyword(ObjectTypeFood)
		...
		basecost = HC_VoreManager_FV_.CheckBasecost(basecost)
...
Function PlayerEatsCorpse()
	...
	If HC_VoreManager_FV_.IsPred() == true
		;BAIL OUT, predator always trumps cannibal
		RETURN
	EndIf
...
Function SendAdrenalineUpdateEvent()
	Var[] args = new Var[1]
	args[0] = playerRef.GetValue(HC_Adrenaline)/KillsForAdrenalinePerkLevel as int
